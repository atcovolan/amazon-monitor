              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    {blockedRecently && (
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-rose-500/15 text-rose-500 border border-rose-500/30 flex items-center gap-1">
                        <AlertTriangle className="w-3 h-3" /> POSSÍVEL BLOQUEIO
                      </span>
                    )}
                  </div>
                ...
                <div className="rounded-2xl overflow-hidden border border-slate-700 dark:border-slate-800 shadow-sm">
                  <div className="flex items-center gap-2 px-4 py-2.5 bg-slate-800">
                    <span className="w-3 h-3 rounded-full bg-rose-500"></span>
                    <span className="w-3 h-3 rounded-full bg-amber-400"></span>
                    <span className="w-3 h-3 rounded-full bg-emerald-500"></span>
                    <span className="ml-2 text-[11px] font-mono text-slate-400">monitor@amazon — consultas em tempo real</span>
                  </div>
                  <div ref={logsBoxRef} className="bg-[#0b1020] max-h-[360px] overflow-y-auto p-4 font-mono text-[12px] leading-relaxed">
                    {logs.length === 0 ? (
                      <span className="text-slate-500">$ aguardando a primeira consulta do monitor...</span>
                    ) : (
                      <>
                        {logs.slice().reverse().map((log, idx) => {
                          const t = new Date(log.time);
                          const stamp = `${t.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })} ${t.toLocaleTimeString('pt-BR')}`;
                          const tag = log.level === 'success' ? 'OK  ' : log.level === 'warning' ? 'WARN' : log.level === 'blocked' ? 'BLOQ' : 'ERRO';
                          const tagColor = log.level === 'success' ? 'text-emerald-400' : log.level === 'warning' ? 'text-amber-400' : 'text-rose-400';
                          const msgColor = log.level === 'success' ? 'text-slate-300' : log.level === 'warning' ? 'text-amber-300' : 'text-rose-300';
                          const shortName = log.name.length > 40 ? log.name.slice(0, 40) + '...' : log.name;
                          return (
                            <div key={idx} className="whitespace-pre-wrap break-words py-0.5">
                              ...
                              <span className="text-sky-300">{shortName}</span>{' '}
                              <span className={msgColor}>&rarr; {log.message}</span>
                            </div>
                          );
                        })}
                      </>
